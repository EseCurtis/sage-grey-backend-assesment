...
// Function to withdraw funds from a user's account by ID
async function withdrawFromUserById(req, res) {
  try {
    const userId = req.body.id;
    const amount = req.body.amount; // Assuming the amount to be withdrawn is sent in the request body
    ...
    console.error('Error withdrawing from user by ID:', error);
    res.status(500).json({ message: 'Internal server error' });
  }
}
